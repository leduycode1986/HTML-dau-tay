...
import { collection, addDoc, serverTimestamp, updateDoc, doc, onSnapshot, getDoc } from 'firebase/firestore'; 
...
function Checkout({ gioHang, setGioHang, userData }) {
  ...
  const handleOrder = async () => {
    ...
    // Logic trừ kho & tạo đơn (giữ nguyên logic chuẩn từ các bước trước)
    const maDonHang = 'MV-' + Math.floor(100000 + Math.random() * 900000);
    await addDoc(collection(db, "donHang"), { maDonHang, khachHang: khach, gioHang, tongTien: tongCong, hinhThucThanhToan: paymentMethod, trangThai: 'Mới đặt', ngayDat: serverTimestamp(), userId: auth.currentUser?.uid || null });
        ...
        <Col md={7}>
          <div className="checkout-box">
            ...
            <Row className="g-3">
              ...
              <Col md={12}><Form.Select value={khach.quanHuyen} onChange={handleSelectShip}><option value="">-- Chọn Quận/Huyện --</option>{dsShip.map((s,i)=><option key={i} value={s.khuVuc}>{s.khuVuc} ({parseInt(s.phi).toLocaleString()}đ)</option>)}</Form.Select></Col>
        ...
        <Col md={5}>
          <div className="checkout-box">
            ...
            <div className="d-flex flex-column gap-2 mb-3">
              {/* Lựa chọn COD */}
                <div className={`payment-option mb-2 ${paymentMethod==='cod' ? 'active' : ''}`} onClick={()=>setPaymentMethod('cod')}>
