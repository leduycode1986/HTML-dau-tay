...
function Home({ dsSanPham, dsDanhMuc, themVaoGio }) {
  ...
  // Các state lọc/sắp xếp giữ nguyên...
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, "banners"), (sn) => setBanners(sn.docs.map(d => ({id: d.id, ...d.data()}))));
    return () => unsub();
  }, []);
  ...
  // ... (Giữ nguyên các hàm lọc sản phẩm và component ProductSlider cũ) ...
    ...
    <Container fluid className="p-0">
      <Row className="g-0">
        ...
        <Col xs={12} md={9} lg={10} className="p-3 p-md-4" style={{background: '#f4f6f9'}}>
                ...
                <div className="banner-slider-container">
                  <Slider {...sliderSettings}>
                    {banners.map(b => (
                      <div key={b.id}>
                        {b.link ? (
                          <Link to={b.link}><img src={b.img} alt="Banner" className="banner-img" /></Link>
                        ) : (
                          <img src={b.img} alt="Banner" className="banner-img" />
                        )}
                      </div>
                    ))}
                  </Slider>
          ...
          {/* (Phần bộ lọc và danh sách sản phẩm giữ nguyên như cũ) */}
