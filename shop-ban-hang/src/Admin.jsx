...
function Admin({ dsSanPham, handleUpdateDS_SP, dsDanhMuc, handleUpdateDS_DM, dsDonHang, handleUpdateStatusOrder, handleDeleteOrder }) {
  ...
  const [shopConfig, setShopConfig] = useState({ 
    tenShop: '', slogan: '', logo: '', diaChi: '', sdt: '', zalo: '', linkFacebook: '', copyright: '', tyLeDiem: 1000, gioiThieu: '', flashSaleEnd: '',
    topBarText: '🚀 Nhận giao hàng miễn phí trong bán kính 5km!' // <-- THÊM MỚI
  });
  ...
  const thongKe = (() => {
    ...
    dsDonHang.forEach(dh => {
      const d = dh.ngayDat?.toDate ? dh.ngayDat.toDate() : null; if(!d) return;
  ...
  const handleLogin = (e) => { e.preventDefault(); if(loginInput.user===adminConfig.user && loginInput.pass===adminConfig.pass) { localStorage.setItem('adminConfig', JSON.stringify({user:adminConfig.user, pass:adminConfig.pass})); setIsLoggedIn(true); } else alert(`Sai mật khẩu! Mặc định là: ${adminConfig.user} / ${adminConfig.pass}`); };
  ...
  if (!isLoggedIn) return (<div className="admin-login-wrapper"><div className="admin-login-card shadow"><h3 className="text-center text-success fw-bold mb-4">QUẢN TRỊ SHOP</h3><Form onSubmit={handleLogin}><Form.Control className="mb-3 p-3" placeholder="Tài khoản" value={loginInput.user} onChange={e=>setLoginInput({...loginInput, user:e.target.value})}/><InputGroup className="mb-4"><Form.Control className="p-3" type={showPass?"text":"password"} placeholder="Mật khẩu" value={loginInput.pass} onChange={e=>setLoginInput({...loginInput, pass:e.target.value})}/><Button variant="outline-secondary" onClick={()=>setShowPass(!showPass)}><i className={showPass?"fa-solid fa-eye-slash":"fa-solid fa-eye"}></i></Button></InputGroup><Button type="submit" variant="success" className="w-100 py-3 fw-bold rounded-pill">ĐĂNG NHẬP</Button></Form></div></div>);

  return (
    <div className="admin-main-container">
      <div className="admin-navbar"><h4>QUẢN TRỊ</h4><div className="d-flex align-items-center gap-3">{shopConfig.sdt && <span className="text-white small d-none d-md-block"><i className="fa-solid fa-phone me-1"></i> {shopConfig.sdt}</span>}<Link to="/"><Button variant="danger" size="sm">Thoát</Button></Link></div></div>
      <Container fluid className="p-3">
        ...
        <Tabs defaultActiveKey="config" className="bg-white p-2 rounded border shadow-sm mb-3">
          <Tab eventKey="config" title="⚙️ CẤU HÌNH">
            <div className="bg-white p-4"><Row><Col md={4} className="text-center"><Form.Label className="fw-bold">Logo</Form.Label><div className="border p-2 mb-2 d-flex align-items-center justify-content-center" style={{height:100}}><img src={shopConfig.logo} style={{maxHeight:'100%'}}/></div><Form.Control type="file" size="sm" onChange={e=>handleUpload(e,'LOGO')}/></Col>
              <Col md={8}><Row className="g-2">
                <Col md={6}><Form.Control placeholder="Tên Shop" value={shopConfig.tenShop} onChange={e=>setShopConfig({...shopConfig, tenShop:e.target.value})}/></Col><Col md={6}><Form.Control placeholder="Slogan" value={shopConfig.slogan} onChange={e=>setShopConfig({...shopConfig, slogan:e.target.value})}/></Col>
                
                {/* --- Ô NHẬP THÔNG BÁO HEADER (MỚI) --- */}
                <Col md={12}><Form.Group><Form.Label className="fw-bold text-success small">Dòng thông báo trên Header (Chạy chữ):</Form.Label><Form.Control value={shopConfig.topBarText} onChange={e=>setShopConfig({...shopConfig, topBarText:e.target.value})}/></Form.Group></Col>
                {/* -------------------------------------- */}
                ...
                <Col md={6}><Form.Control placeholder="Hotline" value={shopConfig.sdt} onChange={e=>setShopConfig({...shopConfig, sdt:e.target.value})}/></Col><Col md={6}><Form.Control placeholder="Zalo" value={shopConfig.zalo} onChange={e=>setShopConfig({...shopConfig, zalo:e.target.value})}/></Col>
                ...
                <Col md={12}><Button variant="success" className="w-100 fw-bold" onClick={luuCauHinh}>LƯU CẤU HÌNH</Button></Col>
              </Row></Col></Row></div>
          </Tab>

          {/* ... (Các tab Banner, Marketing, Product, Menu... GIỮ NGUYÊN CODE CŨ CỦA BẠN - Chỉ cần copy từ file Admin trước vào đây nếu muốn, hoặc dùng code rút gọn nhưng đầy đủ này) */}
          ...
          <Tab eventKey="products" title="📦 SẢN PHẨM"><Button size="sm" className="mb-2 fw-bold" onClick={()=>{setEditData({...editData, sp:null}); setFormDataSP({ ten:'', giaGoc:'', phanTramGiam:0, giaBan:'', donVi:'Cái', moTa:'', anh:'', phanLoai:'', isMoi:false, isKhuyenMai:false, isBanChay:false, isFlashSale:false }); setModal({...modal, sp:true})}}>+ THÊM MỚI</Button><div className="table-responsive"><Table hover bordered size="sm" className="align-middle"><thead className="bg-light"><tr><th>Ảnh</th><th>Tên</th><th>Giá</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody>{dsSanPham.map(sp=><tr key={sp.id}><td><img src={sp.anh||NO_IMAGE} width="40"/></td><td className="fw-bold">{sp.ten}</td><td className="text-danger fw-bold">{sp.giaBan?.toLocaleString()}¥</td><td>{sp.isFlashSale && <Badge bg="warning" text="dark" className="me-1">Flash</Badge>}{sp.isMoi && <Badge bg="success" className="me-1">New</Badge>}{sp.isKhuyenMai && <Badge bg="secondary">Sale</Badge>}</td><td><Button size="sm" variant="warning" className="me-1" onClick={()=>{setEditData({...editData, sp}); setFormDataSP(sp); setModal({...modal, sp:true})}}>✏️</Button><Button size="sm" variant="danger" onClick={()=>{if(confirm('Xóa?')) handleUpdateDS_SP('DELETE',sp.id)}}>🗑️</Button></td></tr>)}</tbody></Table></div></Tab>
          <Tab eventKey="menu" title="📂 DANH MỤC"><Button size="sm" className="mb-2 fw-bold" onClick={()=>{setEditData({...editData, dm:null}); setFormDataDM({ten:'', icon:'', parent:'', order:''}); setModal({...modal, dm:true})}}>+ DANH MỤC</Button><Table bordered size="sm"><tbody>{dsDanhMuc.sort((a,b)=>a.order-b.order).map(d=><tr key={d.id}><td>{d.order}</td><td>{d.parent?'↳ ':''}{d.ten}</td><td>{d.icon}</td><td><Button size="sm" variant="warning" className="me-1" onClick={()=>{setEditData({...editData, dm:d}); setFormDataDM(d); setModal({...modal, dm:true})}}>✏️</Button><Button size="sm" variant="danger" onClick={()=>handleUpdateDS_DM('DELETE',d.id)}>🗑️</Button></td></tr>)}</tbody></Table></Tab>
