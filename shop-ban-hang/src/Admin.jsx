...
function Admin() { 
  ...
  // State hiển thị mật khẩu khi đổi pass
  ...
  const [shopConfig, setShopConfig] = useState({ 
    ...
    paypalClientId: '', // [MỚI] Thêm Client ID PayPal                            
  ...
  const handleExportExcel = () => {
    ...
    if (listToExport.length === 0) return toast.warning("Không có sản phẩm nào để xuất!");
                ...
                <Col md={7}> 
                  <div className="bg-white p-3 rounded shadow-sm border h-100">
                    <div className="d-flex justify-content-between align-items-center border-bottom pb-2 mb-3">
                      ...
                      <Button variant="outline-danger" size="sm" onClick={() => { setFilterStatus('stock_out'); setActiveTab('products_cats'); }}>
                        Xem tất cả
                      </Button>
                    </div>
                    <div style={{ maxHeight: '350px', overflowY: 'auto' }}>
                      <Table hover responsive className="align-middle mb-0">
                        <thead className="bg-light text-secondary small"><tr><th>Sản phẩm</th><th>Danh mục</th><th className="text-center">Tồn kho</th><th>Trạng thái</th></tr></thead>
                        <tbody>
                          {lowStockProducts.length > 0 ? (
                            lowStockProducts.map(sp => {
                              ...
                              return (
                                <tr key={sp.id}>
                                  <td><div className="d-flex align-items-center"><img src={sp.anh || NO_IMAGE} alt="" width="40" height="40" className="rounded border me-2" style={{ objectFit: 'cover' }}/><div><div className="fw-bold small text-dark">{sp.ten}</div><small className="text-muted">{sp.donVi}</small></div></div></td>
                                  <td><Badge bg="light" text="dark" className="border fw-normal">{catName}</Badge></td>
                                  <td className="text-center"><span className={`fw-bold ${sp.soLuong === 0 ? 'text-danger fs-5' : 'text-warning'}`}>{sp.soLuong}</span></td>
                                  <td>{sp.soLuong === 0 ? <Badge bg="danger">Hết hàng</Badge> : <Badge bg="warning" text="dark">Sắp hết</Badge>}</td>
                                </tr>
                              );
                            })
                          ) : (<tr><td colSpan="4" className="text-center py-4 text-muted"><i className="fa-regular fa-circle-check fs-1 text-success mb-2"></i><p className="m-0">Kho hàng đang ổn định!</p></td></tr>)}
                        </tbody>
                      </Table>
                    </div>
                  </div>
                </Col>
                <Col md={5}><div className="bg-white p-3 rounded shadow-sm border h-100"><h6 className="fw-bold text-primary border-bottom pb-2">📦 ĐƠN MỚI NHẤT</h6><div style={{maxHeight:'300px', overflowY:'auto'}}>{dsDonHang.sort((a,b)=>b.ngayDat-a.ngayDat).slice(0,5).map(dh => (<div key={dh.id} className="d-flex justify-content-between border-bottom py-2"><div><strong>{dh.maDonHang}</strong> - {dh.khachHang?.ten}</div><div className="text-success fw-bold">{dh.tongTien?.toLocaleString()} ¥</div></div>))}</div></div></Col>
              </Row>
            </div>
          </Tab>
                ...
                <Row>
                    ...
                    <Card className="shadow-sm border-0">
                      ...
                      <Card.Body className="p-0">
                        <Nav variant="pills" className="flex-column admin-setting-nav">
                          <Nav.Item><Nav.Link eventKey="general_info" className="rounded-0 border-bottom py-3"><i className="fa-solid fa-shop me-2"></i> Thông tin cửa hàng</Nav.Link></Nav.Item>
                          <Nav.Item><Nav.Link eventKey="payment_info" className="rounded-0 border-bottom py-3"><i className="fa-solid fa-credit-card me-2"></i> Ngân hàng & QR</Nav.Link></Nav.Item>
                          <Nav.Item><Nav.Link eventKey="banner_manage" className="rounded-0 border-bottom py-3"><i className="fa-regular fa-images me-2"></i> Quản lý Banner</Nav.Link></Nav.Item>
                          <Nav.Item><Nav.Link eventKey="guide_manage" className="rounded-0 border-bottom py-3"><i className="fa-solid fa-book-open me-2"></i> Hướng dẫn mua hàng</Nav.Link></Nav.Item>
                          <Nav.Item><Nav.Link eventKey="policy_manage" className="rounded-0 border-bottom py-3"><i className="fa-solid fa-shield-halved me-2"></i> Chính sách đổi trả</Nav.Link></Nav.Item>
                          <Nav.Item><Nav.Link eventKey="admin_manage" className="rounded-0 border-bottom py-3"><i className="fa-solid fa-users-gear me-2"></i> Phân quyền Admin</Nav.Link></Nav.Item>
                          <Nav.Item><Nav.Link eventKey="my_account" className="rounded-0 py-3"><i className="fa-solid fa-user-lock me-2"></i> Tài khoản của tôi</Nav.Link></Nav.Item>
                          <Nav.Item><Nav.Link eventKey="member_manage" className="rounded-0 border-bottom py-3"><i className="fa-solid fa-users me-2"></i> Quản lý Thành viên</Nav.Link></Nav.Item>
                          <Nav.Item><Nav.Link eventKey="review_manage" className="rounded-0 border-bottom py-3"><i className="fa-solid fa-star-half-stroke me-2"></i> Quản lý Đánh giá</Nav.Link></Nav.Item>
                        </Nav>
                      </Card.Body>
                    </Card>
                  </Col>
                  ...
                  <Col md={9}>
                    <Tab.Content>
                      ...
                      <div className="bg-white p-4 rounded shadow-sm border">
                        ...
                        <Row className="g-3">
                              ...
                              <Col md={12}>
                                <div className="p-3 bg-light rounded border mt-2">
                                  ...
                                  <Row className="align-items-center">
                                    <Col md={8}><Form.Label className="small text-muted mb-0">Khách hàng mua bao nhiêu <strong>Yên (¥)</strong> thì được tặng <strong>1 điểm</strong>?<br/><span className="fst-italic">(Ví dụ: Nhập 1000 thì khách mua 2.000¥ được 2 điểm)</span></Form.Label></Col>
                                    <Col md={4}><InputGroup size="sm"><Form.Control type="number" className="fw-bold text-primary text-end" value={shopConfig.tyLeDiem || 1000} onChange={e=>setShopConfig({...shopConfig, tyLeDiem: parseInt(e.target.value) || 0})}/><InputGroup.Text className="fw-bold">¥ = 1 điểm</InputGroup.Text></InputGroup></Col>
                                  </Row>
                                </div>
                              </Col>
                            </Row>
                          ...
                          <Col md={4} className="text-center">
                            ...
                            <div className="border rounded p-2 mb-2 d-flex align-items-center justify-content-center" style={{height:'150px', background:'#f8f9fa'}}><img src={shopConfig.logo || NO_IMAGE} style={{maxHeight:'100%', maxWidth:'100%'}} /></div>
                            <Form.Control type="file" size="sm" onChange={e=>handleUpload(e,'LOGO')}/>
                          </Col>
                        </Row>
                        <div className="mt-4 text-end"><Button variant="success" className="fw-bold px-4" onClick={luuCauHinh}>LƯU THÔNG TIN</Button></div>
                      </div>
                      </Tab.Pane>

                      <Tab.Pane eventKey="payment_info">
                        <div className="bg-white p-4 rounded shadow-sm border">
                          <h6 className="text-primary fw-bold border-bottom pb-2 mb-3">THÔNG TIN CHUYỂN KHOẢN & PAYPAL</h6>
                          <Row className="g-3">
                            <Col md={6}>
                              ...
                              <div className="p-3 bg-light rounded border mt-4">
                                <h6 className="fw-bold text-primary small mb-3"><i className="fa-brands fa-paypal me-2"></i> CẤU HÌNH PAYPAL</h6>
                                <Form.Group>
                                  <Form.Label className="fw-bold small">PayPal Client ID</Form.Label>
                                  <Form.Control 
                                    placeholder="Nhập Client ID của bạn..." 
                                    value={shopConfig.paypalClientId || ''} 
                                    onChange={e=>setShopConfig({...shopConfig, paypalClientId:e.target.value})}
                                  />
                                  <Form.Text className="text-muted small">Lấy Client ID tại <a href="https://developer.paypal.com/" target="_blank">developer.paypal.com</a></Form.Text>
                                </Form.Group>
                              </div>
                            </Col>
                            <Col md={6} className="text-center">
                              <Form.Label className="fw-bold small">Ảnh mã QR Ngân hàng</Form.Label>
                      ...
                      {/* Các Tab khác giữ nguyên nội dung (Banner, Hướng dẫn, Chính sách, Admin, Account, Member, Review) */}
                      <Tab.Pane eventKey="banner_manage"><div className="bg-white p-4 rounded shadow-sm border"><h6 className="text-warning fw-bold border-bottom pb-2 mb-3">QUẢN LÝ BANNER SLIDER</h6><div className="mb-4 p-3 bg-light rounded border"><h6 className="small fw-bold text-muted">Thêm Banner Mới</h6><Row className="g-3 align-items-end"><Col md={4}><Form.Label className="small">Ảnh Banner</Form.Label><Form.Control type="file" size="sm" onChange={e=>handleUpload(e,'BANNER')}/></Col><Col md={5}><Form.Label className="small">Link liên kết (Tùy chọn)</Form.Label><Form.Control size="sm" placeholder="VD: /khuyen-mai" value={formBanner.link} onChange={e=>setFormBanner({...formBanner, link:e.target.value})}/></Col><Col md={3}><Button variant="warning" size="sm" className="w-100 fw-bold text-white" onClick={handleAddBanner} disabled={isUploading}>{isUploading ? 'Đang tải...' : '+ THÊM BANNER'}</Button></Col></Row>{formBanner.img && <div className="mt-2"><img src={formBanner.img} height="60" className="rounded border"/></div>}</div><Table hover bordered className="align-middle text-center"><thead className="bg-light"><tr><th>Hình ảnh</th><th>Link dẫn</th><th>Thao tác</th></tr></thead><tbody>{dsBanner.map(b => (<tr key={b.id}><td><img src={b.img} style={{height:'60px', objectFit:'cover', borderRadius:'4px'}} /></td><td className="text-start small text-muted">{b.link || 'Không có link'}</td><td><Button variant="outline-danger" size="sm" onClick={()=>del('banners', b.id)}><i className="fa-solid fa-trash"></i></Button></td></tr>))}{dsBanner.length === 0 && <tr><td colSpan="3" className="text-muted py-3">Chưa có banner nào</td></tr>}</tbody></Table></div></Tab.Pane>
                      <Tab.Pane eventKey="guide_manage"><div className="bg-white p-4 rounded shadow-sm border"><h6 className="text-info fw-bold border-bottom pb-2 mb-3">BIÊN TẬP: HƯỚNG DẪN MUA HÀNG</h6><ReactQuill theme="snow" value={shopConfig.guideContent} onChange={v=>setShopConfig({...shopConfig, guideContent:v})} style={{height:'350px', marginBottom:'50px'}}/><div className="text-end mt-4"><Button variant="success" className="fw-bold px-4" onClick={luuCauHinh}>LƯU NỘI DUNG</Button></div></div></Tab.Pane>
                      <Tab.Pane eventKey="policy_manage"><div className="bg-white p-4 rounded shadow-sm border"><h6 className="text-info fw-bold border-bottom pb-2 mb-3">BIÊN TẬP: CHÍNH SÁCH ĐỔI TRẢ</h6><ReactQuill theme="snow" value={shopConfig.policyContent} onChange={v=>setShopConfig({...shopConfig, policyContent:v})} style={{height:'350px', marginBottom:'50px'}}/><div className="text-end mt-4"><Button variant="success" className="fw-bold px-4" onClick={luuCauHinh}>LƯU NỘI DUNG</Button></div></div></Tab.Pane>
                      <Tab.Pane eventKey="admin_manage"><div className="bg-white p-4 rounded shadow-sm border"><div className="d-flex justify-content-between align-items-center mb-3"><h6 className="text-danger fw-bold m-0">DANH SÁCH QUẢN TRỊ VIÊN</h6></div><InputGroup className="mb-4"><Form.Control placeholder="Nhập email nhân viên muốn cấp quyền..." value={newAdminEmail} onChange={e=>setNewAdminEmail(e.target.value)}/><Button variant="danger" onClick={handleAddAdmin}>+ Cấp quyền</Button></InputGroup><Table hover responsive bordered className="align-middle"><thead className="bg-light"><tr><th>Email Admin</th><th>Quyền hạn</th><th>Thao tác</th></tr></thead><tbody>{adminWhitelist.map((email, i)=>(<tr key={i}><td className="fw-bold text-dark">{email}</td><td><Badge bg="success">Toàn quyền</Badge></td><td className="text-center">{email !== auth.currentUser?.email && (<Button size="sm" variant="outline-danger" onClick={()=>handleRemoveAdmin(email)} title="Xóa quyền"><i className="fa-solid fa-trash"></i></Button>)}</td></tr>))}</tbody></Table><Alert variant="info" className="small mt-3"><i className="fa-solid fa-circle-info me-2"></i> Người có trong danh sách này có thể đăng nhập vào trang Admin. Hiện tại đang cấp quyền "Toàn quyền" (Full Access).</Alert></div></Tab.Pane>
                      <Tab.Pane eventKey="my_account"><div className="bg-white p-4 rounded shadow-sm border" style={{maxWidth:'600px'}}><h6 className="text-dark fw-bold border-bottom pb-2 mb-4">ĐỔI MẬT KHẨU CÁ NHÂN</h6><Form onSubmit={handleUpdatePassword}><Form.Group className="mb-3"><Form.Label className="fw-bold small">Email đăng nhập</Form.Label><Form.Control type="email" value={auth.currentUser?.email || ''} disabled className="bg-light" /></Form.Group><Form.Group className="mb-3"><Form.Label className="fw-bold small">Mật khẩu mới <span className="text-danger">*</span></Form.Label><InputGroup><Form.Control type={showNewPass ? "text" : "password"} value={passData.newPass} onChange={e=>setPassData({...passData, newPass:e.target.value})} placeholder="Nhập mật khẩu mới..." required /><Button variant="outline-secondary" onClick={()=>setShowNewPass(!showNewPass)}><i className={showNewPass ? "fa-solid fa-eye-slash" : "fa-solid fa-eye"}></i></Button></InputGroup></Form.Group><Form.Group className="mb-4"><Form.Label className="fw-bold small">Xác nhận mật khẩu <span className="text-danger">*</span></Form.Label><InputGroup><Form.Control type={showConfirmPass ? "text" : "password"} value={passData.confirmPass} onChange={e=>setPassData({...passData, confirmPass:e.target.value})} placeholder="Nhập lại mật khẩu..." required /><Button variant="outline-secondary" onClick={()=>setShowConfirmPass(!showConfirmPass)}><i className={showConfirmPass ? "fa-solid fa-eye-slash" : "fa-solid fa-eye"}></i></Button></InputGroup></Form.Group><Button type="submit" variant="warning" className="w-100 fw-bold text-white shadow-sm"><i className="fa-solid fa-key me-2"></i> CẬP NHẬT MẬT KHẨU</Button></Form></div></Tab.Pane>
                      <Tab.Pane eventKey="member_manage"><div className="bg-white p-4 rounded shadow-sm border"><div className="d-flex justify-content-between align-items-center mb-4"><h6 className="text-primary fw-bold m-0"><i className="fa-solid fa-users me-2"></i> DANH SÁCH THÀNH VIÊN ({dsUser.length})</h6><div className="input-group" style={{maxWidth: '300px'}}><span className="input-group-text bg-white"><i className="fa-solid fa-magnifying-glass text-muted"></i></span><input type="text" className="form-control border-start-0 ps-0" placeholder="Tìm thành viên..." /></div></div><div className="table-responsive"><Table hover className="align-middle mb-0"><thead className="bg-light text-secondary small text-uppercase"><tr><th>Thành viên</th><th>Liên hệ</th><th>Hạng</th><th>Điểm tích lũy</th><th>Ngày tham gia</th><th>Tác vụ</th></tr></thead><tbody>{dsUser.length > 0 ? dsUser.map(u => (<tr key={u.id}><td><div className="d-flex align-items-center"><div className="rounded-circle bg-light d-flex align-items-center justify-content-center text-primary fw-bold me-3" style={{width:'40px', height:'40px', border:'1px solid #dee2e6'}}>{u.ten ? u.ten.charAt(0).toUpperCase() : 'U'}</div><div><div className="fw-bold text-dark">{u.ten || 'Chưa đặt tên'}</div><small className="text-muted">ID: {u.id.slice(0,6)}...</small></div></div></td><td className="small"><div className="text-dark"><i className="fa-regular fa-envelope me-1"></i> {u.email}</div>{u.sdt && <div className="text-muted mt-1"><i className="fa-solid fa-phone me-1"></i> {u.sdt}</div>}</td><td>{u.diemTichLuy >= 1000 ? (<Badge bg="warning" text="dark" className="border border-warning"><i className="fa-solid fa-crown me-1"></i> VIP</Badge>) : (<Badge bg="light" text="dark" className="border fw-normal">Thành viên</Badge>)}</td><td><strong className="text-success fs-6">{u.diemTichLuy || 0}</strong> <small>điểm</small></td><td className="text-muted small">{u.ngayTao ? new Date(u.ngayTao).toLocaleDateString('vi-VN') : '---'}</td><td><Button variant="outline-primary" size="sm" className="me-1" title="Sửa điểm" onClick={()=>{ setEditData({...editData, user: u}); setUserPoint(u.diemTichLuy || 0); setModal({...modal, user: true}); }}><i className="fa-solid fa-pen-to-square"></i></Button>{adminWhitelist.includes(u.email) ? (<span className="text-muted small ms-2"><i className="fa-solid fa-user-shield"></i> Admin</span>) : (<Button variant="outline-danger" size="sm" onClick={()=>del('users', u.id)} title="Xóa thành viên"><i className="fa-solid fa-trash"></i></Button>)}</td></tr>)) : (<tr><td colSpan="6" className="text-center py-4 text-muted">Chưa có thành viên nào.</td></tr>)}</tbody></Table></div></div></Tab.Pane>
                      <Tab.Pane eventKey="review_manage"><div className="bg-white p-4 rounded shadow-sm border"><div className="d-flex justify-content-between align-items-center mb-4"><h6 className="text-warning fw-bold m-0"><i className="fa-solid fa-comments me-2"></i> ĐÁNH GIÁ TỪ KHÁCH HÀNG ({dsReview.length})</h6></div><div className="table-responsive"><Table hover className="align-middle"><thead className="bg-light text-secondary small text-uppercase"><tr><th style={{width:'30%'}}>Sản phẩm</th><th style={{width:'15%'}}>Khách hàng</th><th style={{width:'15%'}}>Đánh giá</th><th>Nội dung</th><th style={{width:'10%'}}>Xóa</th></tr></thead><tbody>{dsReview.length > 0 ? dsReview.map(rv => { const sp = dsSanPham.find(p => p.id === rv.productId); return (<tr key={rv.id}><td><div className="d-flex align-items-center"><img src={sp?.anh || NO_IMAGE} alt="" width="50" height="50" className="rounded border me-3" style={{objectFit:'cover'}} /><div><div className="fw-bold text-dark text-truncate" style={{maxWidth:'200px'}}>{sp?.ten || 'Sản phẩm đã xóa'}</div><small className="text-muted">ID: {rv.productId.slice(0,6)}...</small></div></div></td><td className="fw-bold text-dark">{rv.name || 'Ẩn danh'}</td><td><div className="text-warning small">{[...Array(5)].map((_, i) => (<i key={i} className={`fa-solid fa-star ${i < rv.rating ? '' : 'text-muted opacity-25'}`}></i>))}</div></td><td><div className="text-secondary fst-italic">"{rv.comment}"</div><small className="text-muted opacity-75 d-block mt-1">{rv.ngay ? new Date(rv.ngay.seconds * 1000).toLocaleDateString('vi-VN') : 'Vừa xong'}</small></td><td><Button variant="outline-danger" size="sm" onClick={() => del('reviews', rv.id)} title="Xóa đánh giá"><i className="fa-solid fa-trash"></i></Button></td></tr>);}) : (<tr><td colSpan="5" className="text-center py-4 text-muted">Chưa có đánh giá nào.</td></tr>)}</tbody></Table></div></div></Tab.Pane>
