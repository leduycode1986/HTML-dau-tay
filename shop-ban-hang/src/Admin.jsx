...
function Admin({ dsSanPham, handleUpdateDS_SP, dsDanhMuc, handleUpdateDS_DM, dsDonHang, handleUpdateStatusOrder, handleDeleteOrder }) {
  ...
  // STATE CẤU HÌNH SHOP (Đã thêm trường gioiThieu)
  const [shopConfig, setShopConfig] = useState({ 
    tenShop: '', slogan: '', logo: '', 
    diaChi: '', sdt: '', linkFacebook: '', copyright: '@2026 Thực phẩm Mai Vàng',
    tyLeDiem: 1000,
    gioiThieu: '' // <-- THÊM TRƯỜNG NÀY
  });
  ...
  return (
    <div className="admin-main-container">
              ...
              <Row>
                ...
                <Col md={8}>
                  <Row>
                    ...
                    {/* --- Ô NHẬP GIỚI THIỆU (MỚI) --- */}
                    <Col md={12}>
                      <Form.Group className="mb-3">
                        <Form.Label className="fw-bold text-success">Giới thiệu cửa hàng (Hiện ở Footer)</Form.Label>
                        <Form.Control 
                          as="textarea" 
                          rows={3} 
                          value={shopConfig.gioiThieu || ''} 
                          onChange={e => setShopConfig({...shopConfig, gioiThieu: e.target.value})} 
                          placeholder="VD: Chuyên cung cấp thực phẩm sạch, tươi ngon, giao hàng nhanh chóng..." 
                        />
                      </Form.Group>
                    </Col>
                    {/* ------------------------------- */}

                    <Col md={12}><Form.Group className="mb-3"><Form.Label className="fw-bold">Địa chỉ Shop</Form.Label><Form.Control as="textarea" rows={2} value={shopConfig.diaChi} onChange={e => setShopConfig({...shopConfig, diaChi: e.target.value})} /></Form.Group></Col>
                    ...
                    <Col md={6}><Form.Group className="mb-3"><Form.Label className="fw-bold">Tỷ lệ điểm (VNĐ đổi 1 điểm)</Form.Label><Form.Control type="number" value={shopConfig.tyLeDiem} onChange={e => setShopConfig({...shopConfig, tyLeDiem: e.target.value})} /></Form.Group></Col>
                    ...
                    <Col md={12}><Form.Group className="mb-3"><Form.Label className="fw-bold">Copyright</Form.Label><Form.Control value={shopConfig.copyright} onChange={e => setShopConfig({...shopConfig, copyright: e.target.value})} /></Form.Group></Col>
                  </Row>
      ...
      {/* (Giữ nguyên phần Modal bên dưới, không thay đổi gì) */}
