...
function Admin() { 
  // --- STATE QUẢN LÝ ĐĂNG NHẬP & QUYỀN (MỚI) ---
  const [isLoggedIn, setIsLoggedIn] = useState(false);
  const [loadingAuth, setLoadingAuth] = useState(true);
  const [loginInput, setLoginInput] = useState({ email: '', pass: '' }); // Đổi user thành email
  ...
  // --- 1. LOGIC XÁC THỰC FIREBASE (THAY THẾ LOGIN CŨ) ---
  useEffect(() => {
    // Lấy whitelist từ Firestore
  ...
  // --- 3. CÁC HÀM XỬ LÝ (GIỮ NGUYÊN 100%) ---
  ...
  // --- 4. CÁC HÀM XỬ LÝ PHÂN QUYỀN MỚI ---
  ...
  const handleRemoveAdmin = async (email) => {
    ...
    if (confirm(`Gỡ quyền truy cập của ${email}?`)) {
  ...
  // --- 7. GIAO DIỆN ---
  if (loadingAuth) return <div className="text-center py-5"><Spinner animation="border" variant="success" /><p className="mt-2">Đang xác thực quyền Admin...</p></div>;
      ...
      <div className="admin-login-card shadow-lg border-0">
        ...
        <Form onSubmit={handleLogin}>
          <Form.Group className="mb-3">
            <Form.Label className="fw-bold">Email Admin</Form.Label>
            <Form.Control className="p-3" type="email" value={loginInput.email} onChange={e=>setLoginInput({...loginInput, email:e.target.value})} placeholder="admin@maivang.com" />
          </Form.Group>
          <Form.Group className="mb-4">
            <Form.Label className="fw-bold">Mật khẩu</Form.Label>
            <InputGroup>
              <Form.Control className="p-3" type={showPass?"text":"password"} value={loginInput.pass} onChange={e=>setLoginInput({...loginInput, pass:e.target.value})} placeholder="******" />
                ...
                <Col md={6} className="border-end">
                  ...
                  <Form.Group className="mb-3 text-center">
                    ...
                    <Form.Label className="btn btn-sm btn-outline-primary" style={{cursor:'pointer'}}>
                      Chọn Logo {isUploading && <Spinner size="sm" animation="border" />} <Form.Control type="file" hidden onChange={e=>handleUpload(e,'LOGO')}/>
                    </Form.Label>
                  </Form.Group>
                  <Row className="g-3">
                  ...
                  <Row className="g-3">
                    ...
                    <Col md={4} className="text-center">
                        <div className="border rounded p-1 mb-2 bg-light">
                            <img src={shopConfig.bankInfo?.qrImage} style={{width:'100%', maxHeight:'150px', objectFit:'contain'}} alt="QR Code" />
                        </div>
              ...
              <div className="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                  <div className="d-flex gap-2">
                      ...
                      <div className="position-relative">
                          <Button variant="outline-primary"><i className="fa-solid fa-file-import me-2"></i> Nhập giá & Kho</Button>
              ...
              <Col md={5}>
                <div className="bg-white p-3 rounded shadow-sm h-100">
                  ...
                  <div style={{maxHeight:400,overflowY:'auto'}}>
                    {dsReview.map(r=><div key={r.id} className="border-bottom py-2"><div className="d-flex justify-content-between"><strong>{r.userName}</strong><small className="text-muted">{r.ngay?.toDate().toLocaleDateString()}</small></div><div className="text-warning small">{'⭐'.repeat(r.rating)}</div><p className="mb-1 small text-secondary">{r.comment}</p><Button size="sm" variant="outline-danger" style={{fontSize:10}} onClick={()=>del('reviews', r.id)}>Xóa</Button></div>)}
                  </div>
                </div>
              </Col>
            </Row>
