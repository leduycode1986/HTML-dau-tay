...
function Admin({ dsSanPham, handleUpdateDS_SP, dsDanhMuc, handleUpdateDS_DM, dsDonHang, handleUpdateStatusOrder, handleDeleteOrder }) {
  ...
  const updateUserPoint = async () => { 
    if(editingUser) {
      await updateDoc(doc(db, "users", editingUser.id), { diemTichLuy: parseInt(userPoint) });
    ...
    <div className="admin-main-container">
      ...
      <Container fluid className="p-3 p-md-4">
        ...
        <Tabs defaultActiveKey="config" className="mb-4 bg-white p-2 rounded shadow-sm border">
              ...
              <Row>
                ...
                <Col md={8}>
                  <Row>
                    ...
                    <Col md={6}><Form.Group className="mb-3"><Form.Label className="fw-bold">Slogan (Khẩu hiệu)</Form.Label><Form.Control value={shopConfig.slogan} onChange={e=>setShopConfig({...shopConfig, slogan:e.target.value})}/></Form.Group></Col>
                    ...
                    <Col md={6}><Form.Group className="mb-3"><Form.Label className="fw-bold">Tỷ lệ điểm (VNĐ/1 điểm)</Form.Label><Form.Control type="number" value={shopConfig.tyLeDiem} onChange={e=>setShopConfig({...shopConfig, tyLeDiem:e.target.value})}/></Form.Group></Col>
          ...
          {/* 2. BANNER (ĐÃ THÊM GHI CHÚ KÍCH THƯỚC) */}
          <Tab eventKey="banner" title="🖼️ BANNER">
            <div className="bg-white p-3 rounded">
              <Row className="align-items-end mb-3">
                <Col md={5}>
                  <Form.Group>
                    <Form.Label className="fw-bold">Chọn ảnh Banner <span className="text-danger small">(Khuyên dùng: 1200 x 400 px)</span></Form.Label>
                    <Form.Control type="file" onChange={e=>handleUpload(e,'BANNER')}/>
                  </Form.Group>
                </Col>
                <Col md={5}>
                  <Form.Group>
                    <Form.Label className="fw-bold">Link khi click (Tùy chọn)</Form.Label>
                    <Form.Control placeholder="/product/..." value={formBanner.link} onChange={e=>setFormBanner({...formBanner, link:e.target.value})}/>
                  </Form.Group>
                </Col>
          ...
          {/* 3. VOUCHER & SHIP (ĐÃ ĐỔI TIỀN TỆ YÊN) */}
          <Tab eventKey="marketing" title="🎟️ VOUCHER & SHIP">
            <Row>
              ...
              <Col md={6} className="p-3">
                <h5 className="text-primary fw-bold border-bottom pb-2">PHÍ VẬN CHUYỂN (THEO KHU VỰC)</h5>
                <div className="d-flex gap-2 mb-3">
          ...
          {/* 4. SẢN PHẨM (KHÔI PHỤC GIAO DIỆN CŨ ĐẸP HƠN) */}
          <Tab eventKey="products" title="📦 SẢN PHẨM">
             <Button variant="primary" className="my-3 fw-bold shadow-sm" onClick={()=>{setEditingSP(null); setFormDataSP({ ten: '', giaGoc: '', phanTramGiam: 0, giaBan: '', donVi: 'Cái', soLuong: 10, moTa: '', anh: '', phanLoai: '', isMoi: false, isKhuyenMai: false, isBanChay: false }); setShowModalSP(true)}}>+ THÊM SẢN PHẨM MỚI</Button>
                   ...
                   return (
                     <tr key={sp.id}>
                       ...
                       <td>
                         ...
                         <Button size="sm" variant="danger" onClick={()=>{if(confirm('Xóa sản phẩm này?')) handleUpdateDS_SP('DELETE', sp.id)}}><i className="fa-solid fa-trash"></i></Button>
                       </td>
                     </tr>
                   )
                 })}</tbody>
          ...
          {/* 6. ĐƠN HÀNG (GIAO DIỆN CŨ) */}
          <Tab eventKey="orders" title={`📋 ĐƠN HÀNG (${dsDonHang.length})`}>
            <div className="table-responsive mt-3">
              <Table hover bordered className="align-middle bg-white shadow-sm">
                <thead className="bg-light"><tr><th>Ngày đặt</th><th>Khách hàng</th><th>Tổng tiền</th><th>Trạng thái</th><th>Thao tác</th></tr></thead>
          ...
          {/* 7. USER & REVIEW (NÂNG CẤP GIAO DIỆN) */}
          <Tab eventKey="users" title="👥 KHÁCH HÀNG & BÌNH LUẬN">
            <Row>
              <Col md={7}>
                 <div className="bg-white p-3 rounded shadow-sm mb-3">
                   ...
                   <div className="table-responsive" style={{maxHeight:400, overflowY:'auto'}}>
                     <Table hover size="sm" className="align-middle">
                       ...
                       <tbody>{dsUser.map(u=>(
                         <tr key={u.id}>
                           <td className="fw-bold">{u.ten}</td>
                           <td>{u.email}</td>
                           <td className="text-warning fw-bold">{u.diemTichLuy}</td>
                           <td>
                             <Button size="sm" variant="outline-primary" className="me-1" onClick={()=>{setEditingUser(u); setUserPoint(u.diemTichLuy); setShowModalUser(true)}}>Sửa điểm</Button>
                             <Button size="sm" variant="outline-danger" onClick={()=>deleteUser(u.id)}>Xóa</Button>
                           </td>
                         </tr>
                       ))}</tbody>
                     </Table>
                       ...
                       return (
                         <div key={r.id} className="border p-3 mb-2 rounded bg-light position-relative">
                           <div className="d-flex justify-content-between align-items-center mb-1">
                             ...
                             <span className="badge bg-secondary" style={{fontSize:10}}>{r.ngay ? r.ngay.toDate().toLocaleDateString() : ''}</span>
                           </div>
          ...
          <Form>
            <Row>
              <Col md={8}>
                ...
                <div className="d-flex gap-3 mt-2 border p-2 rounded bg-light">
                  <Form.Check type="switch" label="Hàng Mới (NEW)" checked={formDataSP.isMoi} onChange={e=>setFormDataSP({...formDataSP,isMoi:e.target.checked})}/>
                  ...
                  <Form.Check type="switch" label="Khuyến Mãi (SALE)" checked={formDataSP.isKhuyenMai} onChange={e=>setFormDataSP({...formDataSP,isKhuyenMai:e.target.checked})}/>
                </div>
              </Col>
              <Col md={4}>
      ...
      {/* MODAL CHI TIẾT ĐƠN HÀNG (GIỮ NGUYÊN GIAO DIỆN ĐẸP CŨ) */}
      ...
      {/* MODAL SỬA USER (MỚI) */}
      <Modal show={showModalUser} onHide={()=>setShowModalUser(false)} centered>
        <Modal.Header closeButton><Modal.Title>Cập nhật Thành Viên</Modal.Title></Modal.Header>
        <Modal.Body>
          <Form.Group><Form.Label>Tên</Form.Label><Form.Control value={editingUser?.ten || ''} disabled /></Form.Group>
          <Form.Group className="mt-2"><Form.Label>Email</Form.Label><Form.Control value={editingUser?.email || ''} disabled /></Form.Group>
          <Form.Group className="mt-2"><Form.Label className="fw-bold text-warning">Điểm tích lũy</Form.Label><Form.Control type="number" value={userPoint} onChange={e=>setUserPoint(e.target.value)} /></Form.Group>
        </Modal.Body>
        <Modal.Footer><Button onClick={updateUserPoint}>Lưu Thay Đổi</Button></Modal.Footer>
      </Modal>
