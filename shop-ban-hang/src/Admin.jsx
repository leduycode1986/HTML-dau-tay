...
function Admin() { 
  ...
  // --- 1. LOGIC XÁC THỰC FIREBASE (CHECK QUYỀN TỪ FIRESTORE) ---
  useEffect(() => {
    // Lấy whitelist admin
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        const whitelist = docSnap.exists() ? (docSnap.data().adminEmails || []) : [];
        
        // Nếu whitelist rỗng hoặc có chứa email -> Cho vào (Để tránh bị khóa ngoài lần đầu)
  ...
  // --- 3. CÁC HÀM XỬ LÝ (GIỮ NGUYÊN TỪ CODE CŨ) ---
  ...
  const onSaveSP = async () => { 
    if (!formDataSP.ten.trim()) return toast.warning("Vui lòng nhập tên!");
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "DanhSachSanPham"); XLSX.writeFile(wb, "Danh_Sach_San_Pham_Update.xlsx");
  };
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const updates = data.map(async (row) => {
            ...
            const updateData = { giaGoc: parseInt(row["Giá gốc"])||0, phanTramGiam: parseInt(row["Phần trăm giảm"])||0, giaBan: Math.floor((parseInt(row["Giá gốc"])||0) * (1 - (parseInt(row["Phần trăm giảm"])||0) / 100)), ten: row["Tên sản phẩm"], soLuong: parseInt(row["Kho"])||0 };
            await updateDoc(doc(db, "sanPham", id), updateData);
        });
        await Promise.all(updates); toast.success("Đã cập nhật xong!"); e.target.value = null; 
      } catch (error) { toast.error("Lỗi đọc file!"); }
    };
    reader.readAsBinaryString(file);
  };

  // --- 5. LOGIC MỚI CHO TAB QUYỀN & MẬT KHẨU ---
  const handleAddAdmin = async () => {
    ...
    setNewAdminEmail(''); toast.success("Đã thêm quyền Admin!");
  };
  ...
  // --- 6. LOGIC BỘ LỌC (GIỮ NGUYÊN) ---
  const filteredProducts = dsSanPham.filter(sp => {
        if (filterCategory) { const subCats = dsDanhMuc.filter(d => d.parent === filterCategory).map(d => d.id); return [filterCategory, ...subCats].includes(sp.phanLoai); }
        return true;
    }).filter(sp => {
    ...
    <div style={{background: '#f8f9fa', minHeight:'100vh'}}>
      <div className="admin-header">
          ...
          <div className="d-flex align-items-center gap-2">
              <span className="text-white-50 small d-none d-md-block">Xin chào, {auth.currentUser?.email}</span>
              <Button variant="danger" size="sm" className="fw-bold px-3" onClick={handleLogout}>Thoát</Button>
          </div>
      </div>
      <Container fluid className="p-3">
        <Tabs defaultActiveKey="dashboard" className="bg-white p-2 rounded border shadow-sm mb-3">
              ...
              <div className="table-responsive mb-3">
                <Table hover bordered className="align-middle">
                  ...
                  <tbody>{currentProducts.map(sp=>(<tr key={sp.id}>
                    ...
                    <td>{sp.isFlashSale && <Badge bg="warning" text="dark" className="me-1">⚡ Sale</Badge>}{sp.phanTramGiam > 0 && <Badge bg="danger" className="me-1">-{sp.phanTramGiam}%</Badge>}{sp.isMoi && <Badge bg="success" className="me-1">New</Badge>}{sp.isBanChay && <Badge bg="info" className="me-1">🔥 Hot</Badge>}</td>
            ...
            <div className="bg-white p-3 rounded shadow-sm">
                ...
                <div className="table-responsive">
                    <Table hover bordered className="align-middle">
                        ...
                        <tbody>{dsDanhMuc.map(d=>(<tr key={d.id} className={d.parent ? 'bg-light' : 'fw-bold'}>
                                    <td style={{width: '80px'}}>{d.order}</td>
                                    <td>{d.parent ? <span className="text-secondary ms-4"><i className="fa-solid fa-turn-up fa-rotate-90 me-2"></i>{d.ten}</span> : <span className="text-success">{d.ten}</span>}</td>
                                    <td className="fs-5">{d.icon}</td>
                                    <td>{d.parent ? <Badge bg="secondary">Danh mục con</Badge> : <Badge bg="primary">Danh mục gốc</Badge>}</td>
                                    <td style={{width: '120px'}}><Button size="sm" variant="warning" className="me-1" onClick={()=>{setEditData({...editData, dm:d}); setFormDM(d); setModal({...modal, dm:true})}}>✏️</Button><Button size="sm" variant="danger" onClick={async()=>{if(confirm('Xóa?')) await deleteDoc(doc(db,"danhMuc",d.id))}}>🗑️</Button></td>
                                </tr>))}</tbody>
                    </Table>
                </div>
            </div>
          ...
          <Tab eventKey="users" title="👥 THÀNH VIÊN & ĐÁNH GIÁ">
            <Row>
              <Col md={7}>
                <div className="bg-white p-3 rounded shadow-sm h-100"><h6 className="fw-bold text-primary border-bottom pb-2">DANH SÁCH THÀNH VIÊN</h6><div className="table-responsive"><Table size="sm" hover><thead><tr><th>Tên</th><th>Email</th><th>Điểm</th><th>Thao tác</th></tr></thead><tbody>{dsUser.map(u=><tr key={u.id}><td>{u.ten}</td><td>{u.email}</td><td className="text-warning fw-bold">{u.diemTichLuy}</td><td><Button size="sm" onClick={()=>{setEditData({...editData, user:u}); setUserPoint(u.diemTichLuy); setModal({...modal, user:true})}}>Sửa</Button></td></tr>)}</tbody></Table></div></div>
              </Col>
              <Col md={5}>
                <div className="bg-white p-3 rounded shadow-sm h-100"><h6 className="fw-bold text-warning border-bottom pb-2">ĐÁNH GIÁ MỚI</h6><div style={{maxHeight:400,overflowY:'auto'}}>{dsReview.map(r=><div key={r.id} className="border-bottom py-2"><div className="d-flex justify-content-between"><strong>{r.userName}</strong><small className="text-muted">{r.ngay?.toDate().toLocaleDateString()}</small></div><div className="text-warning small">{'⭐'.repeat(r.rating)}</div><p className="mb-1 small text-secondary">{r.comment}</p><Button size="sm" variant="outline-danger" style={{fontSize:10}} onClick={()=>del('reviews', r.id)}>Xóa</Button></div>)}</div></div>
              </Col>
            </Row>
          </Tab>
          ...
          {/* TAB 9: QUYỀN & MẬT KHẨU (TÍNH NĂNG MỚI ĐƯỢC THÊM VÀO) */}
          <Tab eventKey="system" title="🔐 QUYỀN & MẬT KHẨU">
             <Row className="p-3 g-4">
                <Col md={6}>
                  <Card className="p-3 shadow-sm h-100">
                    <h6 className="fw-bold text-primary border-bottom pb-2">QUẢN LÝ QUYỀN ADMIN</h6>
                    <InputGroup className="mb-3">
                      <Form.Control placeholder="Nhập email admin mới..." value={newAdminEmail} onChange={e=>setNewAdminEmail(e.target.value)}/>
                      <Button variant="success" onClick={handleAddAdmin}>+ Cấp quyền</Button>
                    </InputGroup>
                    <Table size="sm" bordered hover>
                      <thead className="bg-light"><tr><th>Email Admin</th><th>Xử lý</th></tr></thead>
                      <tbody>
                        {adminWhitelist.map((email, i)=>(<tr key={i}><td>{email}</td><td className="text-center"><Button variant="link" className="text-danger p-0" onClick={()=>handleRemoveAdmin(email)}><i className="fa-solid fa-trash-can"></i></Button></td></tr>))}
                      </tbody>
                    </Table>
                  </Card>
                </Col>
                <Col md={6}>
                  <Card className="p-3 shadow-sm h-100">
                    <h6 className="fw-bold text-warning border-bottom pb-2">ĐỔI MẬT KHẨU</h6>
                    <Form onSubmit={handleUpdatePassword}>
                      <Form.Group className="mb-3">
                        <Form.Label className="small fw-bold">Mật khẩu mới</Form.Label>
                        <Form.Control type="password" value={passData.newPass} onChange={e=>setPassData({...passData, newPass:e.target.value})} required />
                      </Form.Group>
                      <Form.Group className="mb-3">
                        <Form.Label className="small fw-bold">Xác nhận mật khẩu</Form.Label>
                        <Form.Control type="password" value={passData.confirmPass} onChange={e=>setPassData({...passData, confirmPass:e.target.value})} required />
                      </Form.Group>
                      <Button type="submit" variant="warning" className="w-100 fw-bold">CẬP NHẬT NGAY</Button>
                    </Form>
                  </Card>
                </Col>
             </Row>
