...
import { Table, Button, Form, Modal, Badge, Tab, Tabs, Row, Col, Container, InputGroup, Card } from 'react-bootstrap'; // Đã thêm InputGroup, Card
...
// THÊM GIÁ TRỊ MẶC ĐỊNH CHO PROPS ĐỂ TRÁNH LỖI UNDEFINED
function Admin({ dsSanPham = [], handleUpdateDS_SP, dsDanhMuc = [], handleUpdateDS_DM, dsDonHang = [], handleUpdateStatusOrder, handleDeleteOrder }) {
  const [isLoggedIn, setIsLoggedIn] = useState(false);
  const [loginInput, setLoginInput] = useState({ user: '', pass: '' });
  const [showPass, setShowPass] = useState(false); // NÚT ẨN HIỆN MẬT KHẨU

  // TỰ ĐỘNG FIX LỖI ĐĂNG NHẬP
  ...
  const [shopConfig, setShopConfig] = useState({ 
    tenShop:'', slogan:'', logo:'', diaChi:'', sdt:'', zalo:'', linkFacebook:'', copyright:'', tyLeDiem:1000, gioiThieu:'', flashSaleEnd:'', topBarText:'' 
  });
  ...
  const handleUpload = (e, type) => { const f = e.target.files[0]; if(!f) return; const r = new FileReader(); r.onloadend=()=>{ if(type==='LOGO') setShopConfig({...shopConfig,logo:r.result}); if(type==='PRODUCT') setFormDataSP({...formDataSP,anh:r.result}); if(type==='BANNER') setFormBanner({...formBanner,img:r.result}); }; r.readAsDataURL(f); };
  ...
  return (
    <div className="admin-main-container">
      <div className="admin-navbar"><h4>QUẢN TRỊ VIÊN</h4><div className="d-flex align-items-center gap-3">{shopConfig.sdt && <span className="text-white small d-none d-md-block"><i className="fa-solid fa-phone me-1"></i> {shopConfig.sdt}</span>}<Link to="/"><Button variant="danger" size="sm">Thoát</Button></Link></div></div>
      <Container fluid className="p-3">
        <Tabs defaultActiveKey="config" className="bg-white p-2 rounded border shadow-sm mb-3">
          
          <Tab eventKey="config" title="⚙️ CẤU HÌNH"><div className="bg-white p-4"><Row><Col md={4} className="text-center"><Form.Label className="fw-bold">Logo Shop</Form.Label><div className="border p-2 mb-2 d-flex align-items-center justify-content-center" style={{height:100}}><img src={shopConfig.logo} style={{maxHeight:'100%'}}/></div><Form.Control type="file" size="sm" onChange={e=>handleUpload(e,'LOGO')}/></Col><Col md={8}><Row className="g-2"><Col md={12}><Form.Control placeholder="Dòng thông báo Header (Chạy chữ)" value={shopConfig.topBarText} onChange={e=>setShopConfig({...shopConfig, topBarText:e.target.value})}/></Col><Col md={6}><Form.Control placeholder="Tên Shop" value={shopConfig.tenShop} onChange={e=>setShopConfig({...shopConfig, tenShop:e.target.value})}/></Col><Col md={6}><Form.Control placeholder="Slogan" value={shopConfig.slogan} onChange={e=>setShopConfig({...shopConfig, slogan:e.target.value})}/></Col><Col md={12}><Form.Control as="textarea" placeholder="Giới thiệu footer" rows={2} value={shopConfig.gioiThieu} onChange={e=>setShopConfig({...shopConfig, gioiThieu:e.target.value})}/></Col><Col md={6}><Form.Control placeholder="Hotline" value={shopConfig.sdt} onChange={e=>setShopConfig({...shopConfig, sdt:e.target.value})}/></Col><Col md={6}><Form.Control placeholder="Zalo" value={shopConfig.zalo} onChange={e=>setShopConfig({...shopConfig, zalo:e.target.value})}/></Col><Col md={6}><Form.Label className="small fw-bold text-danger">Kết thúc Flash Sale:</Form.Label><Form.Control type="datetime-local" value={shopConfig.flashSaleEnd} onChange={e=>setShopConfig({...shopConfig, flashSaleEnd:e.target.value})}/></Col><Col md={6}><Form.Label className="small fw-bold">Tỷ lệ điểm (¥/1 điểm)</Form.Label><Form.Control type="number" value={shopConfig.tyLeDiem} onChange={e=>setShopConfig({...shopConfig, tyLeDiem:e.target.value})}/></Col><Col md={12}><Form.Control placeholder="Link Facebook" value={shopConfig.linkFacebook} onChange={e=>setShopConfig({...shopConfig, linkFacebook:e.target.value})}/></Col><Col md={12}><Button variant="success" className="w-100 fw-bold" onClick={luuCauHinh}>LƯU CẤU HÌNH</Button></Col></Row></Col></Row></div></Tab>
          
          <Tab eventKey="banner" title="🖼️ BANNER"><div className="bg-white p-3"><div className="d-flex gap-2 mb-3"><Form.Control type="file" onChange={e=>handleUpload(e,'BANNER')}/><Form.Control placeholder="Link khi click..." value={formBanner.link} onChange={e=>setFormBanner({...formBanner,link:e.target.value})}/><Button onClick={()=>{add('banners', formBanner); setFormBanner({img:'', link:''})}}>Thêm</Button></div><div className="d-flex flex-wrap gap-2">{dsBanner.map(b=><div key={b.id} className="position-relative" style={{width:200}}><img src={b.img} className="w-100 rounded"/><Button size="sm" variant="danger" className="position-absolute top-0 end-0" onClick={()=>del('banners', b.id)}>X</Button></div>)}</div></div></Tab>
          
          <Tab eventKey="marketing" title="🎟️ SHIP & COUPON"><Row><Col md={6} className="border-end p-3"><h6 className="fw-bold text-success">MÃ GIẢM GIÁ</h6><div className="d-flex gap-1 mb-2"><Form.Control placeholder="Mã (VD: SALE10)" value={formCoupon.code} onChange={e=>setFormCoupon({...formCoupon,code:e.target.value.toUpperCase()})}/><Form.Control type="number" placeholder="Giảm (¥)" value={formCoupon.giamGia} onChange={e=>setFormCoupon({...formCoupon,giamGia:e.target.value})}/><Button size="sm" onClick={()=>{add('coupons',formCoupon); setFormCoupon({code:'',giamGia:0})}}>Thêm</Button></div><Table size="sm"><tbody>{dsCoupon.map(c=><tr key={c.id}><td>{c.code}</td><td>{parseInt(c.giamGia).toLocaleString()}¥</td><td><Button size="sm" variant="danger" onClick={()=>del('coupons',c.id)}>X</Button></td></tr>)}</tbody></Table></Col><Col md={6} className="p-3"><h6 className="fw-bold text-primary">PHÍ SHIP (KHU VỰC)</h6><div className="d-flex gap-1 mb-2"><Form.Control placeholder="Khu vực (VD: Quận 1)" value={formShip.khuVuc} onChange={e=>setFormShip({...formShip,khuVuc:e.target.value})}/><Form.Control type="number" placeholder="Phí (¥)" value={formShip.phi} onChange={e=>setFormShip({...formShip,phi:e.target.value})}/><Button size="sm" onClick={()=>{add('shipping',formShip); setFormShip({khuVuc:'',phi:0})}}>Thêm</Button></div><Table size="sm"><tbody>{dsShip.map(s=><tr key={s.id}><td>{s.khuVuc}</td><td>{parseInt(s.phi).toLocaleString()}¥</td><td><Button size="sm" variant="danger" onClick={()=>del('shipping',s.id)}>X</Button></td></tr>)}</tbody></Table></Col></Row></Tab>
          
          <Tab eventKey="products" title="📦 SẢN PHẨM"><Button size="sm" className="mb-2 fw-bold" onClick={()=>{setEditData({...editData, sp:null}); setFormDataSP({ ten: '', giaGoc: '', phanTramGiam: 0, giaBan: '', donVi: 'Cái', moTa: '', anh: '', phanLoai: '', isMoi: false, isKhuyenMai: false, isBanChay: false, isFlashSale: false }); setModal({...modal, sp:true})}}>+ THÊM SẢN PHẨM</Button><div className="table-responsive"><Table hover bordered size="sm" className="align-middle"><thead className="bg-light"><tr><th>Ảnh</th><th>Tên</th><th>Giá</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody>{dsSanPham.map(sp=><tr key={sp.id}><td><img src={sp.anh||NO_IMAGE} width="40"/></td><td className="fw-bold">{sp.ten}</td><td className="text-danger fw-bold">{sp.giaBan?.toLocaleString()}¥</td><td>{sp.isFlashSale && <Badge bg="warning" text="dark" className="me-1">⚡ Flash</Badge>}{sp.isMoi && <Badge bg="success" className="me-1">New</Badge>}{sp.isBanChay && <Badge bg="danger" className="me-1">Hot</Badge>}{sp.isKhuyenMai && <Badge bg="secondary">Sale</Badge>}</td><td><Button size="sm" variant="warning" className="me-1" onClick={()=>{setEditData({...editData, sp}); setFormDataSP(sp); setModal({...modal, sp:true})}}>✏️</Button><Button size="sm" variant="danger" onClick={()=>{if(confirm('Xóa?')) handleUpdateDS_SP('DELETE',sp.id)}}>🗑️</Button></td></tr>)}</tbody></Table></div></Tab>
      ...
      {/* MODAL SP */}
      <Modal show={modal.sp} onHide={()=>setModal({...modal,sp:false})} size="lg" centered><Modal.Header closeButton><Modal.Title>{editData.sp?'Cập nhật':'Thêm mới'}</Modal.Title></Modal.Header><Modal.Body><Form><Row><Col md={8}><Form.Control className="mb-2" placeholder="Tên SP" value={formDataSP.ten} onChange={e=>setFormDataSP({...formDataSP,ten:e.target.value})}/><Form.Select className="mb-2" value={formDataSP.phanLoai} onChange={e=>setFormDataSP({...formDataSP,phanLoai:e.target.value})}><option value="">Danh mục</option>{dsDanhMuc.map(d=><option key={d.id} value={d.id}>{d.parent?'-- ':''}{d.ten}</option>)}</Form.Select><div className="d-flex gap-2 mb-2"><Form.Control type="number" placeholder="Giá gốc" value={formDataSP.giaGoc} onChange={e=>setFormDataSP({...formDataSP,giaGoc:e.target.value})}/><Form.Control type="number" placeholder="% Giảm" value={formDataSP.phanTramGiam} onChange={e=>setFormDataSP({...formDataSP,phanTramGiam:e.target.value})}/></div><Form.Control className="mb-2 bg-light fw-bold text-danger" readOnly value={formDataSP.giaBan}/><div className="d-flex flex-wrap gap-3 border p-2 rounded"><Form.Check type="switch" label="⚡ FLASH SALE" className="fw-bold text-warning" checked={formDataSP.isFlashSale} onChange={e=>setFormDataSP({...formDataSP,isFlashSale:e.target.checked})}/><Form.Check type="switch" label="New" checked={formDataSP.isMoi} onChange={e=>setFormDataSP({...formDataSP,isMoi:e.target.checked})}/><Form.Check type="switch" label="Hot" checked={formDataSP.isBanChay} onChange={e=>setFormDataSP({...formDataSP,isBanChay:e.target.checked})}/><Form.Check type="switch" label="Sale" checked={formDataSP.isKhuyenMai} onChange={e=>setFormDataSP({...formDataSP,isKhuyenMai:e.target.checked})}/></div></Col><Col md={4}><Form.Control type="file" onChange={e=>handleUpload(e,'PRODUCT')}/><img src={formDataSP.anh||NO_IMAGE} className="w-100 mt-2 border rounded"/></Col></Row><ReactQuill theme="snow" value={formDataSP.moTa} onChange={v=>setFormDataSP({...formDataSP,moTa:v})} className="mt-2"/></Form></Modal.Body>
        <Modal.Footer><Button variant="secondary" onClick={()=>setModal({...modal,sp:false})}>Hủy</Button><Button onClick={()=>{handleUpdateDS_SP(editData.sp?'UPDATE':'ADD', formDataSP); setModal({...modal,sp:false})}}>Lưu</Button></Modal.Footer>
      </Modal>
      {/* MODAL DM */}
