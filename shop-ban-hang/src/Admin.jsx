      ...
      <Container fluid className="p-3">
        ...
        <Tabs defaultActiveKey="config" className="bg-white p-2 rounded border shadow-sm mb-3">
              ...
              <Row>
                <Col md={4} className="text-center">
                  <Form.Group className="mb-3">
                    ...
                    <div className="border p-2 mb-2 d-flex align-items-center justify-content-center" style={{height:100}}><img src={shopConfig.logo} style={{maxHeight:'100%'}}/></div>
                    <Form.Control type="file" size="sm" onChange={e=>handleUpload(e,'LOGO')}/>
                  </Form.Group>
                </Col>
                <Col md={8}>
                  <Row className="g-2">
                    <Col md={12}><Form.Group><Form.Label className="fw-bold">Dòng thông báo Header (Chạy chữ)</Form.Label><Form.Control value={shopConfig.topBarText} onChange={e=>setShopConfig({...shopConfig, topBarText:e.target.value})}/></Form.Group></Col>
                    ...
                    <Col md={6}><Form.Group><Form.Label className="fw-bold text-danger">Kết thúc Flash Sale</Form.Label><Form.Control type="datetime-local" value={shopConfig.flashSaleEnd} onChange={e=>setShopConfig({...shopConfig, flashSaleEnd:e.target.value})}/></Form.Group></Col>
                    <Col md={6}><Form.Group><Form.Label className="fw-bold">Tỷ lệ điểm (¥/1 điểm)</Form.Label><Form.Control type="number" value={shopConfig.tyLeDiem} onChange={e=>setShopConfig({...shopConfig, tyLeDiem:e.target.value})}/></Form.Group></Col>
          ...
          <Tab eventKey="banner" title="🖼️ BANNER">
            <div className="bg-white p-3">
              <div className="d-flex gap-2 mb-3 align-items-end">
                <Form.Group className="flex-grow-1"><Form.Label className="fw-bold">Chọn ảnh</Form.Label><Form.Control type="file" onChange={e=>handleUpload(e,'BANNER')}/></Form.Group>
                <Form.Group className="flex-grow-1"><Form.Label className="fw-bold">Link</Form.Label><Form.Control placeholder="/san-pham/..." value={formBanner.link} onChange={e=>setFormBanner({...formBanner,link:e.target.value})}/></Form.Group>
                <Button onClick={()=>{add('banners', formBanner); setFormBanner({img:'', link:''})}}>Thêm</Button>
              </div>
              <div className="d-flex flex-wrap gap-2">{dsBanner.map(b=><div key={b.id} className="position-relative" style={{width:200}}><img src={b.img} className="w-100 rounded"/><Button size="sm" variant="danger" className="position-absolute top-0 end-0" onClick={()=>del('banners', b.id)}>X</Button></div>)}</div>
            </div>
          </Tab>
          ...
          <Tab eventKey="marketing" title="🎟️ SHIP & COUPON">
            <Row>
              <Col md={6} className="border-end p-3">
                <h6 className="fw-bold text-success mb-3">MÃ GIẢM GIÁ</h6>
                <div className="d-flex gap-2 mb-2 align-items-end">
                  <Form.Group><Form.Label className="small fw-bold">Mã Code</Form.Label><Form.Control value={formCoupon.code} onChange={e=>setFormCoupon({...formCoupon,code:e.target.value.toUpperCase()})}/></Form.Group>
                  <Form.Group><Form.Label className="small fw-bold">Giảm (¥)</Form.Label><Form.Control type="number" value={formCoupon.giamGia} onChange={e=>setFormCoupon({...formCoupon,giamGia:e.target.value})}/></Form.Group>
              ...
              <Col md={6} className="p-3">
                <h6 className="fw-bold text-primary mb-3">PHÍ SHIP (KHU VỰC)</h6>
                <div className="d-flex gap-2 mb-2 align-items-end">
                  <Form.Group><Form.Label className="small fw-bold">Khu vực</Form.Label><Form.Control value={formShip.khuVuc} onChange={e=>setFormShip({...formShip,khuVuc:e.target.value})}/></Form.Group>
                  <Form.Group><Form.Label className="small fw-bold">Phí (¥)</Form.Label><Form.Control type="number" value={formShip.phi} onChange={e=>setFormShip({...formShip,phi:e.target.value})}/></Form.Group>
          ...
          <Tab eventKey="products" title="📦 SẢN PHẨM">
            <Button size="sm" className="mb-2 fw-bold" onClick={()=>{setEditData({...editData, sp:null}); setFormDataSP({ ten: '', giaGoc: '', phanTramGiam: 0, giaBan: '', donVi: 'Cái', moTa: '', anh: '', phanLoai: '', isMoi: false, isKhuyenMai: false, isBanChay: false, isFlashSale: false }); setModal({...modal, sp:true})}}>+ THÊM SẢN PHẨM MỚI</Button>
            <div className="table-responsive"><Table hover bordered size="sm" className="align-middle"><thead className="bg-light"><tr><th>Ảnh</th><th>Tên</th><th>Giá</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody>{dsSanPham.map(sp=><tr key={sp.id}><td><img src={sp.anh||NO_IMAGE} width="40"/></td><td className="fw-bold">{sp.ten}</td><td className="text-danger fw-bold">{sp.giaBan?.toLocaleString()}¥</td><td>{sp.isFlashSale && <Badge bg="warning" text="dark" className="me-1">⚡ Flash</Badge>}{sp.isMoi && <Badge bg="success" className="me-1">New</Badge>}{sp.isBanChay && <Badge bg="danger" className="me-1">Hot</Badge>}{sp.isKhuyenMai && <Badge bg="secondary">Sale</Badge>}</td><td><Button size="sm" variant="warning" className="me-1" onClick={()=>{setEditData({...editData, sp}); setFormDataSP(sp); setModal({...modal, sp:true})}}>✏️</Button><Button size="sm" variant="danger" onClick={()=>{if(confirm('Xóa?')) handleUpdateDS_SP('DELETE',sp.id)}}>🗑️</Button></td></tr>)}</tbody></Table></div>
          </Tab>
          
          <Tab eventKey="menu" title="📂 DANH MỤC">
            <Button size="sm" className="mb-2 fw-bold" onClick={()=>{setEditData({...editData, dm:null}); setFormDM({ten:'', icon:'', parent:'', order:''}); setModal({...modal, dm:true})}}>+ THÊM DANH MỤC</Button>
            <Table bordered size="sm" hover>
              <thead className="bg-light"><tr><th>TT</th><th>Tên</th><th>Icon</th><th>Thao tác</th></tr></thead>
              <tbody>{dsDanhMuc.sort((a,b)=>a.order-b.order).map(d=><tr key={d.id}><td>{d.order}</td><td>{d.parent?'↳ ':''}{d.ten}</td><td>{d.icon}</td><td><Button size="sm" variant="warning" className="me-1" onClick={()=>{setEditData({...editData, dm:d}); setFormDM(d); setModal({...modal, dm:true})}}>✏️</Button><Button size="sm" variant="danger" onClick={()=>handleUpdateDS_DM('DELETE',d.id)}>🗑️</Button></td></tr>)}</tbody>
            </Table>
          </Tab>
          ...
          <Tab eventKey="orders" title={`📋 ĐƠN HÀNG (${dsDonHang.length})`}>
            <div className="table-responsive">
              <Table hover bordered size="sm" className="align-middle">
                <thead className="bg-light"><tr><th>Mã</th><th>Ngày</th><th>Khách</th><th>Tổng</th><th>TT</th><th>Xử lý</th></tr></thead>
          ...
          <Tab eventKey="users" title="👥 USER & REVIEW">
            <Row>
              <Col md={7}>
                <h6 className="fw-bold text-primary mb-3">DANH SÁCH THÀNH VIÊN</h6>
              ...
              <Col md={5}>
                <h6 className="fw-bold text-warning mb-3">BÌNH LUẬN MỚI</h6>
                <div style={{maxHeight:400,overflowY:'auto'}}>{dsReview.map(r=><div key={r.id} className="border p-2 mb-2 bg-light rounded"><div className="d-flex justify-content-between"><strong>{r.userName}</strong><small>{r.ngay?.toDate().toLocaleDateString()}</small></div><div className="text-warning">{'⭐'.repeat(r.rating)}</div><p className="mb-1 bg-white p-1 border">{r.comment}</p><Button size="sm" variant="danger" onClick={()=>delReview(r.id)}>Xóa</Button></div>)}</div>
              </Col>
            </Row>
          </Tab>
        </Tabs>
