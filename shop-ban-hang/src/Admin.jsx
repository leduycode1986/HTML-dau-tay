...
function Admin({ dsSanPham, handleUpdateDS_SP, dsDanhMuc, handleUpdateDS_DM, dsDonHang, handleUpdateStatusOrder, handleDeleteOrder }) {
  ...
  const [shopConfig, setShopConfig] = useState({ 
    ...
    sdt: '', zalo: '', // Tách riêng Hotline & Zalo
    linkFacebook: '', copyright: '', tyLeDiem: 1000, gioiThieu: '',
    flashSaleEnd: '' // Cấu hình thời gian Flash Sale
  });
  ...
  return (
    <div className="admin-main-container">
      ...
      <Container fluid className="p-3 p-md-4">
        ...
        <Tabs defaultActiveKey="config" className="mb-4 bg-white p-2 rounded shadow-sm border">
              ...
              <Row>
                ...
                <Col md={8}>
                  <Row>
                    ...
                    <Col md={6}><Form.Group className="mb-3"><Form.Label className="fw-bold text-danger">Kết thúc Flash Sale lúc:</Form.Label><Form.Control type="datetime-local" value={shopConfig.flashSaleEnd || ''} onChange={e=>setShopConfig({...shopConfig, flashSaleEnd:e.target.value})} /></Form.Group></Col>
          ...
          <Tab eventKey="products" title="📦 SẢN PHẨM">
             <Button variant="primary" className="my-3 fw-bold shadow-sm" onClick={()=>{setEditingSP(null); setFormDataSP({ ten: '', giaGoc: '', phanTramGiam: 0, giaBan: '', donVi: 'Cái', soLuong: 10, moTa: '', anh: '', phanLoai: '', isMoi: false, isKhuyenMai: false, isBanChay: false, isFlashSale: false }); setShowModalSP(true)}}>+ THÊM SẢN PHẨM</Button>
                 ...
                 <tbody>{dsSanPham.map(sp=>{
                   ...
                   return (
                     <tr key={sp.id}>
                       ...
                       <td><Button size="sm" variant="warning" className="me-1" onClick={()=>{setEditingSP(sp); setFormDataSP(sp); setShowModalSP(true)}}><i className="fa-solid fa-pen"></i></Button><Button size="sm" variant="danger" onClick={()=>{if(confirm('Xóa?')) handleUpdateDS_SP('DELETE', sp.id)}}><i className="fa-solid fa-trash"></i></Button></td>
                     </tr>
                   )
                 })}</tbody>
               </Table>
      ...
      <div className="d-flex flex-wrap gap-3 mt-2 border p-2 rounded bg-light">
        ...
        <Form.Check type="switch" label="Hàng Mới" checked={formDataSP.isMoi} onChange={e=>setFormDataSP({...formDataSP,isMoi:e.target.checked})}/>
        <Form.Check type="switch" label="Bán Chạy" checked={formDataSP.isBanChay} onChange={e=>setFormDataSP({...formDataSP,isBanChay:e.target.checked})}/>
