...
function Admin({ dsSanPham, handleUpdateDS_SP, dsDanhMuc, handleUpdateDS_DM, dsDonHang, handleUpdateStatusOrder, handleDeleteOrder }) {
  ...
  // Logic Sắp xếp Phân cấp (VD: 18.1, 18.2)
  const sortedDanhMuc = (() => {
  ...
  return (
    <div className="admin-main-container">
      <div className="admin-navbar">
        <h4 className="m-0 fw-bold">QUẢN TRỊ SHOP</h4>
        ...
        <Tabs defaultActiveKey="products" className="mb-4 bg-white p-2 rounded shadow-sm border">
          <Tab eventKey="products" title="📦 SẢN PHẨM">
            ...
            <Table hover responsive className="bg-white border rounded align-middle">
              <thead className="table-light"><tr><th>Ảnh</th><th>Tên sản phẩm</th><th>Giá</th><th>Thao tác</th></tr></thead>
              <tbody>{(dsSanPham || []).map(sp => (
                <tr key={sp.id}>
                  <td><img src={sp.anh || NO_IMAGE} className="admin-thumb" width="50" height="50" style={{objectFit: 'cover'}} alt="" /></td>
                  <td><div className="fw-bold">{sp.ten}</div>{sp.isMoi && <Badge bg="success">NEW</Badge>} {sp.isBanChay && <Badge bg="danger">HOT</Badge>}</td>
      ...
      {/* MODAL SẢN PHẨM & DANH MỤC (GIỮ NGUYÊN NHƯ BẢN FULL TRƯỚC) */}
      <Modal show={showModalSP} onHide={() => setShowModalSP(false)} size="lg" centered>
         ...
         <Modal.Body>
           ...
           <Row><Col><Form.Group className="mb-3"><Form.Label>Giá gốc</Form.Label><Form.Control type="number" value={formDataSP.giaGoc} onChange={e=>setFormDataSP({...formDataSP, giaGoc: e.target.value})} /></Form.Group></Col><Col><Form.Group className="mb-3"><Form.Label>% Giảm</Form.Label><Form.Control type="number" value={formDataSP.phanTramGiam} onChange={e=>setFormDataSP({...formDataSP, phanTramGiam: e.target.value})} /></Form.Group></Col></Row>
           <Form.Group className="mb-3"><Form.Label>Ảnh</Form.Label><Form.Control type="file" onChange={handleImageUpload} /></Form.Group>
           <div className="mb-3 d-flex gap-3"><Form.Check type="switch" label="Mới" checked={formDataSP.isMoi} onChange={e=>setFormDataSP({...formDataSP, isMoi: e.target.checked})} /><Form.Check type="switch" label="Hot" checked={formDataSP.isBanChay} onChange={e=>setFormDataSP({...formDataSP, isBanChay: e.target.checked})} /></div>
      ...
      <Modal show={showModalDM} onHide={() => setShowModalDM(false)} centered>
        ...
        <Modal.Body>
          ...
          <Col><Form.Group className="mb-3"><Form.Label>Icon</Form.Label><Form.Select value={formDataDM.icon} onChange={e=>setFormDataDM({...formDataDM, icon: e.target.value})}><option value="">None</option>{ICON_LIST.map(i=><option key={i} value={i}>{i}</option>)}</Form.Select></Form.Group></Col></Row>
          ...
          <Button variant="success" className="w-100" onClick={() => { handleUpdateDS_DM(editingDM ? 'UPDATE' : 'ADD', formDataDM); setShowModalDM(false); }}>LƯU MENU</Button>
        </Modal.Body>
      </Modal>
    </div>
  );
