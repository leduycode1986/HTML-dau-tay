    ...
    <Container className="py-5">
      <div className="detail-container shadow-lg bg-white rounded-4 overflow-hidden border-0">
        ...
        <div className="detail-info-box p-5">
          <div className="mb-3">{sp.isMoi && <Badge bg="success" className="me-2 px-3 py-2">SẢN PHẨM MỚI</Badge>}{sp.isBanChay && <Badge bg="danger" className="px-3 py-2">BÁN CHẠY</Badge>}</div>
          ...
          <div className="bg-light p-4 rounded-4 mb-4 text-center border">
            ...
            <div className="text-muted small mt-2">Đơn vị: {sp.donVi || 'Cái'}</div>
          </div>
          <div className="mb-4 lh-lg text-secondary fs-5 quill-content" dangerouslySetInnerHTML={{ __html: sp.moTa || 'Đang cập nhật mô tả...' }}></div>
          <Button variant="success" size="lg" className="w-100 py-3 fw-bold shadow rounded-pill" onClick={() => themVaoGio(sp)}>THÊM VÀO GIỎ HÀNG</Button>
        </div>
      </div>
    </Container>
  );
