      ...
      <Navbar expand="lg" sticky="top" className="navbar-custom navbar-dark">
        <Container fluid> 
          ...
          <Navbar.Collapse>
            ...
            <Form className="d-flex w-50 mx-auto"><Form.Control placeholder="Tìm kiếm..." className="search-input" value={tuKhoa} onChange={e=>setTuKhoa(e.target.value)} /></Form>
      ...
      <Container fluid className="p-0">
        <Row className="g-0">
            <Col lg={2} className="d-none d-lg-block sidebar-container">
                ...
                <div className={`category-item ${danhMucHienTai==='all'?'active':''}`} onClick={()=>setDanhMucHienTai('all')}>🏠 Tất cả sản phẩm</div>
                {dsDanhMuc.filter(d=>!d.parent).map(dm => (
                    <div key={dm.id}><div className={`category-item ${danhMucHienTai===(dm.customId||dm.id)?'active':''}`} onClick={()=>setDanhMucHienTai(dm.customId||dm.id)}>{dm.icon} {dm.ten}</div></div>
                ))}
            </Col>
