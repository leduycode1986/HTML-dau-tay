    ...
    <Container className="py-5 bg-white shadow-sm mt-3 rounded">
      ...
      <Row className="g-5">
        <Col lg={7}>
            ...
            <tbody>{gioHang.map(i => (
              <tr key={i.id}>
                ...
                <td><div className="d-flex align-items-center gap-2">
                  <Button variant="link" size="sm" onClick={()=>chinhSuaSoLuong(i.id, 'giam')}>-</Button>
                  <span>{i.soLuong}</span>
                  <Button variant="link" size="sm" onClick={()=>chinhSuaSoLuong(i.id, 'tang')}>+</Button>
                </div></td>
          ...
          <div className="map-container"><iframe title="m" src={`https://maps.google.com/maps?q=${encodeURIComponent(khach.diachi || 'Vietnam')}&output=embed`}></iframe></div>
        </Col>
        <Col lg={5}>
          <div className="p-4 bg-light rounded border sticky-top" style={{top: '20px'}}>
            <h5 className="fw-bold mb-4">THANH TOÁN</h5>
            <div className="d-flex justify-content-between h4 fw-bold text-danger pt-3 border-top">
              <span>TỔNG CỘNG:</span><span>{tongTien.toLocaleString()} ¥</span>
            </div>
