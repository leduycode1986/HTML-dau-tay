...
function Cart({ gioHang, handleDatHang }) {
  const [khach, setKhach] = useState({ ten: '', sdt: '', email: '', diachi: '', ghiChu: '' });
  ...
  return (
    <Container className="py-5">
      <Row className="g-5">
        <Col lg={7}>
          <h4 className="fw-bold text-success mb-4 text-uppercase">Thông tin giao hàng</h4>
          ...
          <Form.Control className="mb-3 p-3" placeholder="Địa chỉ đầy đủ *" value={khach.diachi} onChange={e => setKhach({...khach, diachi: e.target.value})} />
        ...
        <Col lg={5}>
          <div className="p-4 bg-light rounded border shadow-sm sticky-top" style={{top: '20px'}}>
            ...
            <Button variant="success" className="w-100 py-3 mt-4 fw-bold" onClick={() => handleDatHang(khach)}>ĐẶT HÀNG</Button>
          </div>
        </Col>
      </Row>
    </Container>
