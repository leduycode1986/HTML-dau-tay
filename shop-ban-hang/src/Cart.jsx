...
import { collection, onSnapshot, addDoc, serverTimestamp, updateDoc, doc } from 'firebase/firestore'; // Cập nhật import
...
function Cart({ gioHang, dsDanhMuc, handleDatHang: propsHandleDatHang, chinhSuaSoLuong, xoaSanPham, currentUser, userData }) {
  ...
  const handleSelectShip = (e) => { const kv = e.target.value; setKhach({...khach, quanHuyen: kv}); const foundShip = dsShip.find(s => s.khuVuc === kv); setShippingFee(foundShip ? parseInt(foundShip.phi) : 0); };
  const tongCong = tamTinh + shippingFee - (appliedCoupon ? parseInt(appliedCoupon.giamGia) : 0);

  // --- LOGIC ĐẶT HÀNG MỚI VỚI MÃ ĐƠN CHUYÊN NGHIỆP ---
  const onDatHang = async () => {
    if (!khach.ten || !khach.sdt || !khach.diachi) return toast.warning("Vui lòng điền đầy đủ thông tin giao hàng!");
    
    // Tạo mã đơn hàng ngẫu nhiên: MV + 6 số (VD: MV-839201)
    const maDonHang = 'MV-' + Math.floor(100000 + Math.random() * 900000);

    // Xử lý tích điểm
    if (currentUser && userData) {
      // Lấy tỷ lệ điểm từ config (nếu có, ở đây giả sử 1000)
      const tyLe = 1000; 
    ...
    await addDoc(collection(db, "donHang"), { 
      maDonHang: maDonHang, // <-- LƯU MÃ MỚI
      khachHang: khach, 
      gioHang, 
      tongTien: tongCong, 
      trangThai: 'Mới đặt', 
    ...
    toast.success(`Đặt hàng thành công! Mã đơn của bạn là: ${maDonHang}`);
    // Gọi hàm clear giỏ hàng từ props hoặc reload (ở đây giả sử propsHandleDatHang xử lý clear)
    // Để đơn giản, ta reload trang hoặc gọi props nếu props hỗ trợ clear
    // Tạm thời gọi props cũ để clear state ở App
