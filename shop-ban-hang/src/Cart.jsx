...
function Cart({ gioHang, dsDanhMuc, handleDatHang, chinhSuaSoLuong, xoaSanPham, currentUser, userData }) {
  ...
  // Lấy dữ liệu Ship & Coupon từ Firebase
  useEffect(() => {
  ...
  const applyCoupon = () => {
    ...
    if(found) {
      ...
      toast.success(`Áp dụng mã ${found.code} giảm ${parseInt(found.giamGia).toLocaleString()}đ`);
    } else {
  ...
  return (
    <Container fluid className="py-4">
      <Row>
        <Col lg={2} className="d-none d-lg-block"><div className="sidebar-main shadow-sm bg-white rounded overflow-hidden"><div className="bg-success text-white p-3 fw-bold text-center text-uppercase"><i className="fa-solid fa-bars me-2"></i> DANH MỤC</div><div className="category-list p-2">{dsDanhMuc.filter(d => !d.parent).map(parent => {const hasChild = dsDanhMuc.some(c => c.parent === parent.id);const isOpen = openMenuId === parent.id;return (<div key={parent.id} className="mb-1 border-bottom"><div className="d-flex align-items-center justify-content-between p-2 text-dark"><Link to={`/category/${parent.id}`} className="text-decoration-none text-dark flex-grow-1 d-flex align-items-center"><span className="me-2">{parent.icon || '📦'}</span> {parent.ten}</Link>{hasChild && <span onClick={(e) => { e.preventDefault(); setOpenMenuId(isOpen ? null : parent.id); }} style={{cursor: 'pointer', padding: '0 10px'}}>{isOpen ? '▲' : '▼'}</span>}</div>{hasChild && isOpen && <div className="ms-3 ps-2 border-start bg-light rounded">{dsDanhMuc.filter(c => c.parent === parent.id).map(child => (<Link key={child.id} to={`/category/${child.id}`} className="d-block py-1 px-2 text-decoration-none text-secondary small hover-green">↳ {child.ten}</Link>))}</div>}</div>);})}</div></div></Col>

        <Col lg={6}>
          <div className="bg-white shadow-sm p-4 rounded-4 mb-3"><h5 className="fw-bold text-success mb-4 text-uppercase border-bottom pb-2">1. Danh sách sản phẩm</h5><Table hover responsive className="align-middle"><tbody>{gioHang.map(i => (<tr key={i.id}><td><img src={i.anh} width="60" height="60" style={{objectFit:'cover', borderRadius:'8px'}} alt=""/></td><td><div className="fw-bold">{i.ten}</div><small className="text-muted">{i.giaBan?.toLocaleString()} ¥</small></td><td><div className="d-flex align-items-center gap-2 border rounded-pill px-2" style={{width:'fit-content'}}><Button variant="link" size="sm" className="text-dark fw-bold text-decoration-none p-0" onClick={()=>chinhSuaSoLuong(i.id, 'giam')}>-</Button><span className="mx-1">{i.soLuong}</span><Button variant="link" size="sm" className="text-dark fw-bold text-decoration-none p-0" onClick={()=>chinhSuaSoLuong(i.id, 'tang')}>+</Button></div></td><td className="text-end fw-bold text-danger">{(i.giaBan * i.soLuong).toLocaleString()} ¥</td><td><Button variant="link" className="text-danger p-0" onClick={()=>xoaSanPham(i.id)}>🗑️</Button></td></tr>))}</tbody></Table></div>
          ...
          <div className="bg-white shadow-sm p-4 rounded-4">
            ...
            <Form.Select className="mb-3 p-3 bg-light border-0" value={khach.quanHuyen} onChange={handleSelectShip}>
               <option value="">-- Chọn Quận/Huyện để tính Ship --</option>
               {dsShip.map((s, idx) => <option key={idx} value={s.khuVuc}>{s.khuVuc} (+{parseInt(s.phi).toLocaleString()}đ)</option>)}
            </Form.Select>
            
            <Form.Control as="textarea" rows={2} className="mb-3 p-3 bg-light border-0" placeholder="Địa chỉ chi tiết (Số nhà, đường...)" value={khach.diachi} onChange={e => setKhach({...khach, diachi: e.target.value})} />
        ...
        <Col lg={4}>
          <div className="cart-summary-box">
            ...
            <div className="input-group mb-3">
               <Form.Control placeholder="Mã giảm giá" value={couponInput} onChange={e=>setCouponInput(e.target.value)} />
               <Button variant="outline-primary" onClick={applyCoupon}>Áp dụng</Button>
            </div>
            {appliedCoupon && <div className="alert alert-success p-2 small mb-3">Đã dùng mã: <strong>{appliedCoupon.code}</strong> (-{parseInt(appliedCoupon.giamGia).toLocaleString()})</div>}
