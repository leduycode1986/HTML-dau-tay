      ...
      <Row>
        ...
        <Col lg={6}>
          ...
          <div className="bg-white shadow-sm p-4 rounded-4">
            <h5 className="fw-bold text-success mb-4 text-uppercase border-bottom pb-2">2. Giao hàng</h5>
            {!currentUser && <Alert variant="warning" className="small">Bạn chưa đăng nhập. Hãy <Link to="/auth" className="fw-bold">đăng nhập</Link> để tích điểm!</Alert>}<Form.Control className="mb-3 p-3 bg-light border-0" placeholder="Họ tên *" value={khach.ten} onChange={e => setKhach({...khach, ten: e.target.value})} /><Form.Control className="mb-3 p-3 bg-light border-0" placeholder="SĐT *" value={khach.sdt} onChange={e => setKhach({...khach, sdt: e.target.value})} />
            <Form.Select className="mb-3 p-3 bg-light border-0" value={khach.quanHuyen} onChange={handleSelectShip}><option value="">-- Chọn Khu Vực Giao Hàng --</option>{dsShip.map((s, idx) => (<option key={idx} value={s.khuVuc}>{s.khuVuc} (Ship: {parseInt(s.phi).toLocaleString()} ¥)</option>))}</Form.Select>
            <Form.Control as="textarea" rows={2} className="mb-3 p-3 bg-light border-0" placeholder="Địa chỉ chi tiết..." value={khach.diachi} onChange={e => setKhach({...khach, diachi: e.target.value})} /><Form.Control as="textarea" rows={2} className="mb-3 p-3 bg-light border-0" placeholder="Ghi chú..." value={khach.ghiChu} onChange={e => setKhach({...khach, ghiChu: e.target.value})} />
          </div>
        </Col>
