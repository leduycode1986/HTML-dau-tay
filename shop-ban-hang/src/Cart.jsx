...
function Cart({ gioHang, dsDanhMuc, handleDatHang: propsHandleDatHang, chinhSuaSoLuong, xoaSanPham, currentUser, userData }) {
  ...
  const applyCoupon = () => {
    ...
    if(found) { setAppliedCoupon(found); toast.success(`Đã giảm ${parseInt(found.giamGia).toLocaleString()}đ`); } else { toast.error("Mã không đúng!"); }
  };
  ...
  const onDatHang = async () => {
    if (!khach.ten || !khach.sdt || !khach.diachi || !khach.quanHuyen) return toast.warning("Vui lòng nhập đủ thông tin và chọn khu vực giao hàng!");
    ...
    if (currentUser && userData) {
      ...
      toast.info(`Cộng ${diemCong} điểm!`);
    }

    await addDoc(collection(db, "donHang"), { 
      maDonHang, khachHang: khach, gioHang, tongTien: tongCong, trangThai: 'Mới đặt', ngayDat: serverTimestamp(), userId: currentUser ? currentUser.uid : null 
    });
  ...
  if (gioHang.length === 0) return (<Container className="py-5 text-center"><div className="p-5 bg-white rounded-4 shadow-sm"><h1 className="display-1 text-muted mb-4">🛒</h1><h3 className="fw-bold text-dark mb-3">Giỏ hàng trống!</h3><Link to="/"><Button variant="success" size="lg" className="rounded-pill px-5">MUA SẮM NGAY</Button></Link></div></Container>);
  ...
  return (
    <Container fluid className="py-4">
      <Row>
        ...
        <Col lg={6}><div className="bg-white shadow-sm p-4 rounded-4 mb-3"><h5 className="fw-bold text-success mb-4 text-uppercase border-bottom pb-2">1. Sản phẩm</h5><Table hover responsive className="align-middle"><tbody>{gioHang.map(i => (<tr key={i.id}><td><img src={i.anh} width="60" height="60" style={{objectFit:'cover', borderRadius:'8px'}}/></td><td><Link to={`/san-pham/${toSlug(i.ten)}/${i.id}`} className="fw-bold text-decoration-none text-dark">{i.ten}</Link><div className="small text-muted">{i.giaBan?.toLocaleString()} ¥</div></td><td><div className="d-flex align-items-center gap-2 border rounded-pill px-2" style={{width:'fit-content'}}><Button variant="link" size="sm" className="text-dark fw-bold text-decoration-none p-0" onClick={()=>chinhSuaSoLuong(i.id, 'giam')}>-</Button><span className="mx-1">{i.soLuong}</span><Button variant="link" size="sm" className="text-dark fw-bold text-decoration-none p-0" onClick={()=>chinhSuaSoLuong(i.id, 'tang')}>+</Button></div></td><td className="text-end fw-bold text-danger">{(i.giaBan * i.soLuong).toLocaleString()} ¥</td><td><Button variant="link" className="text-danger p-0" onClick={()=>xoaSanPham(i.id)}>🗑️</Button></td></tr>))}</tbody></Table></div><div className="bg-white shadow-sm p-4 rounded-4"><h5 className="fw-bold text-success mb-4 text-uppercase border-bottom pb-2">2. Giao hàng</h5>{!currentUser && <Alert variant="warning" className="small">Bạn chưa đăng nhập. Hãy <Link to="/auth" className="fw-bold">đăng nhập</Link> để tích điểm!</Alert>}<Form.Control className="mb-3 p-3 bg-light border-0" placeholder="Họ tên *" value={khach.ten} onChange={e => setKhach({...khach, ten: e.target.value})} /><Form.Control className="mb-3 p-3 bg-light border-0" placeholder="SĐT *" value={khach.sdt} onChange={e => setKhach({...khach, sdt: e.target.value})} /><Form.Select className="mb-3 p-3 bg-light border-0" value={khach.quanHuyen} onChange={handleSelectShip}><option value="">-- Chọn Khu Vực Giao Hàng --</option>{dsShip.map((s, idx) => (<option key={idx} value={s.khuVuc}>{s.khuVuc} (Ship: {parseInt(s.phi).toLocaleString()} ¥)</option>))}</Form.Select><Form.Control as="textarea" rows={2} className="mb-3 p-3 bg-light border-0" placeholder="Địa chỉ chi tiết..." value={khach.diachi} onChange={e => setKhach({...khach, diachi: e.target.value})} /><Form.Control as="textarea" rows={2} className="mb-3 p-3 bg-light border-0" placeholder="Ghi chú..." value={khach.ghiChu} onChange={e => setKhach({...khach, ghiChu: e.target.value})} /></div></Col>
        <Col lg={4}><div className="cart-summary-box"><h5 className="fw-bold mb-4 border-bottom pb-3">TỔNG KẾT</h5><div className="input-group mb-3"><Form.Control placeholder="Mã giảm giá" value={couponInput} onChange={e=>setCouponInput(e.target.value)} /><Button variant="outline-primary" onClick={applyCoupon}>Áp dụng</Button></div>{appliedCoupon && <div className="alert alert-success p-2 small mb-3">Đã dùng mã: <strong>{appliedCoupon.code}</strong> (-{parseInt(appliedCoupon.giamGia).toLocaleString()})</div>}<div className="d-flex justify-content-between mb-2"><span className="text-muted">Tạm tính:</span><span className="fw-bold">{tamTinh.toLocaleString()} ¥</span></div><div className="d-flex justify-content-between mb-2"><span className="text-muted">Phí vận chuyển:</span><span className="text-dark fw-bold">+{shippingFee.toLocaleString()} ¥</span></div>{appliedCoupon && <div className="d-flex justify-content-between mb-2 text-success"><span className="text-muted">Giảm giá:</span><span>-{parseInt(appliedCoupon.giamGia).toLocaleString()} ¥</span></div>}<div className="d-flex justify-content-between h3 fw-bold text-danger pt-3 border-top mb-4"><span>TỔNG CỘNG:</span><span>{tongCong.toLocaleString()} ¥</span></div><Button variant="success" size="lg" className="w-100 py-3 fw-bold rounded-pill shadow mb-3" onClick={onDatHang}>XÁC NHẬN ĐẶT HÀNG</Button><Link to="/"><Button variant="outline-secondary" className="w-100 py-2 fw-bold rounded-pill">← MUA THÊM SP KHÁC</Button></Link></div></Col>
      </Row>
    </Container>
  );
}
